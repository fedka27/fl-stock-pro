...
import static com.stockroompro.models.columns.AdvertisementColumns.ACTIVE;

/**
 * Created by bagach.alexandr on 25.05.15.
 */
public class EditAdvertisementRequest extends UpdateDatabaseRequest<NewAdvert, AdvertisementContentValuesConverter> implements RequestWithNotifications {

    public EditAdvertisementRequest(Context context, Bundle bundle) {
    ...
    public ResponseHolder makeRequest(long date) throws Exception {
        return Communicator.getAppServer().editAdvertisement(getBundle().getLong(AD_ID),
                new RequestParams()
                        ...
                        .addParameter(ACTIVE, getBundle().getInt(ACTIVE))
                        ...
                        .buildJSONWithPathParams(AppServerSpecs.ADVERTISEMENT_DETAILS_PATH, AppServerSpecs.ADVERTISEMENT_ID, getBundle().getLong(AD_ID)));
    }
    ...
    public void processResponse(ResponseHolder<NewAdvert> responseHolder) throws Exception {
        ...
        advertisement.setType(getBundle().getLong(ADVERT_TYPE));
        ...
        advertisement.setFilters(getBundle().getIntegerArrayList(ADVERT_OPTIONS));
        advertisement.setPhones(getBundle().getStringArrayList(ADVERT_PHONES));
        if (getBundle().getString(CITY_NAME) != null) {
            String[] cityName = getBundle().getString(CITY_NAME).split(",");
            if (cityName.length > 1) {
                advertisement.setCityName(cityName[1]);
            }
        }
